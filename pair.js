...
const videos = [
        "https://raw.githubusercontent.com/NjabuloJf/njabulo-data/main/njabulovideo/njabulovideo1.mp4",
        "https://raw.githubusercontent.com/NjabuloJf/njabulo-data/main/njabulovideo/njabulovideo2.mp4",
        "https://raw.githubusercontent.com/NjabuloJf/njabulo-data/main/njabulovideo/njabulovideo3.mp4",
        "https://raw.githubusercontent.com/NjabuloJf/njabulo-data/main/njabulovideo/njabulovideo4.mp4",
        "https://raw.githubusercontent.com/NjabuloJf/njabulo-data/main/njabulovideo/njabulovideo5.mp4",
    ];
...
const config = {
    ...
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029VbDAxWX6WaKuHuMJov3C',
    BOT_NAME: 'Njabulo JB MiniBot',
    DEV_NAME: 'ɳʝαႦυʅσ ʝႦ',
    REACTION_EMOJI: '💙',
    REACTION_THROTTLE: 5000
};

// ========== GITHUB TOKEN - WITH ERROR HANDLING ==========
let octokit;
try {
    octokit = new Octokit({ 
    ...
    console.log('⚠️ GitHub init failed, continuing without GitHub sync');
    octokit = null;
}
...
async function cleanDuplicateFiles(number) {
    ...
    try {
        ...
        console.log(`⚠️ GitHub error for ${number}:`, error.message);
    }
}
...
async function setupStatusHandlers(socket) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        try {
            if (config.AUTO_RECORDING && message.key.remoteJid) {
...
// ========== AUTO REACT STATUS ==========
async function setupAutoReactStatus(socket) {
    let lastReactionTime = 0;
    const reactionEmoji = config.REACTION_EMOJI || '💙';
    const throttleTime = config.REACTION_THROTTLE || 5000;
    ...
    socket.ev.on('messages.upsert', async (m) => {
        ...
        for (const message of messages) {
            if (message.key && message.key.remoteJid === "status@broadcast") {
                console.log("📱 Detected status update from:", message.key.participant || message.key.remoteJid);

                const now = Date.now();
                if (now - lastReactionTime < throttleTime) {
                    console.log("⏳ Throttling reactions to prevent overflow.");
                    continue;
                }

                const botJid = socket.user && socket.user.id ? socket.user.id.split(":")[0] + "@s.whatsapp.net" : null;
                if (!botJid) {
                ...
                try {
                    ...
                    console.log(`✅ Successfully reacted to status update with ${reactionEmoji}`);
                    await delay(2000);
                } catch (error) {
                    console.error('❌ Auto-react status error:', error.message);
                }
            }
        }
    });
...
// ========== AUTO VIEW STATUS ==========
async function setupAutoViewStatus(socket) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        const message = messages[0];
        ...
        try {
            if (config.AUTO_VIEW_STATUS) {
                ...
                while (retries > 0) {
                    try {
                        ...
                        retries--;
                        console.warn(`Failed to view status, retries left: ${retries}`, error);
                        if (retries === 0) throw error;
                        await delay(1000);
                    }
                }
            ...
            console.error('Auto-view status error:', error.message);
        }
    });
}
...
function setupCommandHandlers(socket, number) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (command === 'autoreact' || command === 'ar') {
            ...
            if (subCmd === 'on') {
                ...
                return await socket.sendMessage(from, {
                    image: { url: getRandomImage() },
                    caption: '❌ *Auto-react status disabled!*\n\nI will not react to status updates.'
                }, { quoted: msg });
            } else {
                const status = config.AUTO_REACT_STATUS ? '🟢 ON' : '🔴 OFF';
                await socket.sendMessage(sender, { react: { text: '❤️', key: msg.key } });
                return await socket.sendMessage(from, {
                    image: { url: getRandomImage() },
                    caption: `📌 *Auto-React Status Settings*\n\nStatus: ${status}\n\n*Commands:*\n${config.PREFIX}autoreact on - Enable auto-react\n${config.PREFIX}autoreact off - Disable auto-react`
                }, { quoted: msg });
            }
        }

        // ========== AUTO VIEW STATUS COMMAND ==========
        if (command === 'autoview' || command === 'av') {
            const subCmd = args[0]?.toLowerCase();
            ...
            if (subCmd === 'on') {
                ...
                return await socket.sendMessage(from, {
                    image: { url: getRandomImage() },
                    caption: '✅ *Auto-view status enabled!*\n\nI will automatically view status updates.'
                }, { quoted: msg });
            } else if (subCmd === 'off') {
                ...
                return await socket.sendMessage(from, {
                    image: { url: getRandomImage() },
                    caption: '❌ *Auto-view status disabled!*\n\nI will not view status updates.'
                }, { quoted: msg });
            } else {
                const status = config.AUTO_VIEW_STATUS ? '🟢 ON' : '🔴 OFF';
                await socket.sendMessage(sender, { react: { text: '👁️', key: msg.key } });
                return await socket.sendMessage(from, {
                    image: { url: getRandomImage() },
                    caption: `📌 *Auto-View Status Settings*\n\nStatus: ${status}\n\n*Commands:*\n${config.PREFIX}autoview on - Enable auto-view\n${config.PREFIX}autoview off - Disable auto-view`
                }, { quoted: msg });
            }
        }
        ...
        const fakevCard = {
            ...
            message: {
                contactMessage: {
                    displayName: `© ${config.DEV_NAME || 'ɳʝαႦυʅσ ʝႦ'} `,
...
// ========== KEEP ALIVE - PREVENT DISCONNECTION ==========
...
function setupAutoRestart(socket, number) {
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log(`✅ Connection re-established for ${number}`);
        }
    });
}
...
async function EmpirePair(number, res) {
    ...
    try {
        ...
        // ========== SETUP KEEP ALIVE ==========
        const keepAliveInterval = setupKeepAlive(socket);
        // ======================================

        // ========== SETUP ALL HANDLERS ==========
        setupAutoReactStatus(socket);
        setupAutoViewStatus(socket);
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                try {
                    ...
                    const connectedMessage = `╭─╣ *ɴᴊᴀʙᴜʟᴏ ᴊʙ ᴍɪɴɪʙᴏᴛ* ╠⁠┈
